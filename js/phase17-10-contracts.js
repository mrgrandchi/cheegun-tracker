...
const CONTRACTS=[
 ...
 {type:"rescue",icon:"◉",name:"SURVIVOR EXTRACTION",desc:"Locate and escort a trapped survivor to safety.",targetTypes:["residential","emergency"],difficulty:3,credits:700,xp:460,rep:3,bonus:"New Survivor"},
...
function complete({reason="OBJECTIVE_COMPLETE"}={}){const s=load(),c=s.active;if(!c)return{ok:false,reason:"NO_ACTIVE_CONTRACT"};const p=window.CheegunState?.load?.();if(p){p.credits=(p.credits||0)+c.credits;p.xp=(p.xp||0)+c.xp;p.traderRep=(p.traderRep||0)+c.rep;window.CheegunState.save(p)}s.rep+=c.rep;if(c.type==="rescue")window.CheegunSettlement?.recruit?.({source:"CONTRACT:"+c.name});window.CheegunFactions?.applyContract?.(c,"COMPLETE");s.completed.push({...c,status:"COMPLETE",completedAt:Date.now(),reason});s.active=null;save(s);return{ok:true,contract:c,rewards:{credits:c.credits,xp:c.xp,rep:c.rep}}}
function fail(reason="EXPEDITION_FAILED"){const s=load();if(!s.active)return{ok:false,reason:"NO_ACTIVE_CONTRACT"};window.CheegunFactions?.applyContract?.(s.active,"FAILED");s.failed.push({...s.active,status:"FAILED",failedAt:Date.now(),reason});s.active=null;save(s);return{ok:true}}
