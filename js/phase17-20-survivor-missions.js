...
function finish(id,force=null){const s=load(),m=s.active.find(x=>x.id===id);if(!m)return{ok:false,reason:"MISSION_NOT_FOUND"};if(force===null&&Date.now()<m.endsAt)return{ok:false,reason:"MISSION_IN_PROGRESS"};s.active=s.active.filter(x=>x.id!==id);const success=force===null?Math.random()*100<m.chance:!!force;m.status=success?"SUCCESS":"FAILED";if(success){s.completed++;applySuccess(m)}else{s.failed++;applyFailure(m);window.CheegunCasualtySystem?.missionFailure?.(m)}window.CheegunSurvivorCommunity?.missionOutcome?.(m,success);window.CheegunSurvivorLegacy?.missionOutcome?.(m,success);window.CheegunSurvivorProgression?.missionOutcome?.(m,success);window.CheegunSurvivorSynergy?.record?.("MISSION_"+(success?"SUCCESS":"FAILURE"),m.team||[]);window.CheegunSurvivorEquipment?.wear?.(m.team||[],success?6:14);s.history.push({type:m.status,id,mission:m.type,at:Date.now()});save(s);return{ok:true,success,mission:m}}
