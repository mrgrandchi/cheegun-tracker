...
function recover(){const s=load(),p=window.CheegunState?.load?.();if(!s.injury)return{ok:false,reason:"NO_INJURY"};const factionDiscount=window.CheegunFactions?.modifier?.("medical")?.treatmentDiscount||0;const settlementDiscount=window.CheegunSettlement?.bonuses?.().medic||0;const infraDiscount=window.CheegunInfrastructure?.bonuses?.().treatmentDiscount||0;const discount=Math.min(.70,factionDiscount+settlementDiscount+infraDiscount);const cost=Math.max(0,Math.round(s.injury.creditCost*(1-discount)));if((p?.credits||0)<cost)return{ok:false,reason:"INSUFFICIENT_CREDITS",cost};p.credits-=cost;window.CheegunState.save(p);s.injury=null;s.recoveryUntil=0;save(s);return{ok:true,cost}}
