...
function repair(id){const g=GEAR[id],p=window.CheegunState?.load?.(),s=load();if(!g||!s.owned.includes(id))return{ok:false,reason:"NOT_OWNED"};const cur=s.durability[id]??g.durability,missing=g.durability-cur,cost=Math.ceil(missing*2);if(missing<=0)return{ok:false,reason:"FULL_DURABILITY"};if((p.credits||0)<cost)return{ok:false,reason:"INSUFFICIENT_CREDITS"};p.credits-=cost;s.durability[id]=g.durability;window.CheegunState.save(p);save(s);return{ok:true,cost,durability:g.durability}}
