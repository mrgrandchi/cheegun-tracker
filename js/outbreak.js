...
function phase16b10StartExpedition(){
 ...
 if(result?.ok){log("REAL WORLD EXPEDITION • "+result.runId.toUpperCase());document.dispatchEvent(new CustomEvent("cheegunExpeditionRuntimeStarted",{detail:{runId:result.runId,position:[...player],extraction:result.extraction||null}}));}
 return result;
}
...
function move(points){const token=++moveToken;moving=true;selected=false;document.dispatchEvent(new CustomEvent("cheegunMovementStarted",{detail:{from:[...player],points:points.map(x=>[x.lat??x[0],x.lng??x[1]])}}));const p=simplify(points);let seg=0;clearVisuals();const next=()=>{if(token!==moveToken)return;if(seg>=p.length-1){moving=false;log("ARRIVED AT DESTINATION");discover();reveal();hud();document.dispatchEvent(new CustomEvent("cheegunMovementArrived",{detail:{position:[...player]}}));return}const a=L.latLng(player),b=p[++seg],d=Math.max(1,a.distanceTo(b)),terrain=terrainAt(a),cfg=TERRAIN[terrain]||TERRAIN.open;
...
function killZombie(z){
 ...
 map.removeLayer(z.marker);map.removeLayer(z.awareness);log("INFECTED NEUTRALIZED");document.dispatchEvent(new CustomEvent("cheegunCombatKill",{detail:{id:z.id,position:[...z.pos]}}));
}
function attack(){
...
window.CheegunOutbreakRuntime={
  version:"18.3",
  status:()=>({player:[...player],moving,gameOver,health,hunger,thirst,stamina,infection,minutes,discovered:found.size,searched:searched.size,inventory:inv().length,zombies:zombies.length,expedition:window.CheegunRealWorldMode?.status?.(L.latLng(player))||null}),
  playerPosition:()=>L.latLng(player)
};
document.dispatchEvent(new CustomEvent("cheegunOutbreakRuntimeReady",{detail:{version:"18.3"}}));
