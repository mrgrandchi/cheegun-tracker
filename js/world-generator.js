...
function summary(world){
 ...
 world.features.forEach(f=>{if(f.type==="road")s.roads++;else if(f.type==="building")s.buildings++;else if(f.type==="water")s.water++;else if(f.type==="forest")s.forests++;else if(f.type==="residential")s.residential++;else if(f.type==="industrial")s.industrial++});
 return s;
}
