...
function blocked(p){return !!TERRAIN[terrainAt(p)]?.blocked}
...
function redrawFog(){if(fogLayer)map.removeLayer(fogLayer);const b=L.latLngBounds(WORLD),sw=b.getSouthWest(),ne=b.getNorthEast(),cells=[],rows=18,cols=28,dLat=(ne.lat-sw.lat)/rows,dLng=(ne.lng-sw.lng)/cols;for(let r=0;r<rows;r++)for(let c=0;c<cols;c++){const center=L.latLng(sw.lat+(r+.5)*dLat,sw.lng+(c+.5)*dLng);let v=0;for(const e of explored)v=Math.max(v,clamp(1-dist(center,e)/visionRadius,0,1));cells.push(L.rectangle([[sw.lat+r*dLat,sw.lng+c*dLng],[sw.lat+(r+1)*dLat,sw.lng+(c+1)*dLng]],{pane:"overlayPane",stroke:false,fillColor:"#020504",fillOpacity:v>.82?.05:v>.18?.32:.7,interactive:false}))}fogLayer=L.layerGroup(cells).addTo(map);fogLayer.bringToFront();currentVision.bringToFront()}
function reveal(){const last=explored.at(-1);if(!last||dist(last,player)>52)explored.push([...player]);explored=explored.slice(-52);currentVision.setLatLng(player);redrawFog()}
...
function move(points){const token=++moveToken;moving=true;selected=false;const p=simplify(points);let seg=0;clearVisuals();const next=()=>{if(token!==moveToken)return;if(seg>=p.length-1){moving=false;log("ARRIVED AT DESTINATION");discover();reveal();hud();return}const a=L.latLng(player),b=p[++seg],d=Math.max(1,a.distanceTo(b)),terrain=terrainAt(a),cfg=TERRAIN[terrain]||TERRAIN.open,dur=Math.max(700,d/(3*cfg.speed)),start=performance.now();const frame=now=>{if(token!==moveToken)return;const t=clamp((now-start)/dur,0,1),q=L.latLng(a.lat+(b.lat-a.lat)*t,a.lng+(b.lng-a.lng)*t);if(blocked(q)){moving=false;log("ROUTE BLOCKED • PATH ABORTED");return}player=[q.lat,q.lng];survivor.setLatLng(player);const mins=Math.max(.005,d/dur*.0167);minutes+=mins;stamina=clamp(stamina-mins*(terrain==="forest"?.22:terrain==="road"?.08:.12),0,100);discover();reveal();hud();if(t<1)requestAnimationFrame(frame);else next()};requestAnimationFrame(frame)};next()}
